...
class واجهة_التليجرام:
    def __init__(self, token, admin_id, bot_state, guardian_ref):
        ...
        settings_conv_handler = ConversationHandler(entry_points=[MessageHandler(filters.Regex('^⚙️ الإعدادات$'),self.settings_menu)],states={SELECTING_SETTING:[CallbackQueryHandler(self.ask_for_new_value,pattern='^change_')],TYPING_VALUE:[MessageHandler(filters.TEXT & ~filters.COMMAND,self.set_new_value)],},fallbacks=[CallbackQueryHandler(self.start_callback,pattern='^main_menu$')],)
    ...
    def _get_main_keyboard(self):
        ...
        keyboard=[[KeyboardButton("📊 الحالة"),KeyboardButton(pause_button_text)],[KeyboardButton("💰 بيع يدوي"),KeyboardButton("⚙️ الإعدادات")],[KeyboardButton("🔬 التشخيص"),KeyboardButton(debug_button_text)]]
    ...
    async def start(self,update:Update,context:ContextTypes.DEFAULT_TYPE):
        ...
        await message_to_reply.reply_text('<b>أهلاً بك في مركز قيادة صياد الدرر!</b>',reply_markup=self._get_main_keyboard(),parse_mode=ParseMode.HTML)
    ...
    async def settings_menu(self,update:Update,context:ContextTypes.DEFAULT_TYPE):
        s=self.bot_state
        keyboard=[[InlineKeyboardButton(f"💵 مبلغ الشراء ({s['BUY_AMOUNT_BNB']} BNB)",callback_data='change_BUY_AMOUNT_BNB')],[InlineKeyboardButton(f"🚀 إكرامية الغاز ({s['GAS_PRICE_TIP_GWEI']} Gwei)",callback_data='change_GAS_PRICE_TIP_GWEI')],[InlineKeyboardButton(f"📊 الانزلاق ({s['SLIPPAGE_LIMIT']}%)",callback_data='change_SLIPPAGE_LIMIT')],[InlineKeyboardButton(f"💧 حد السيولة ({s['MINIMUM_LIQUIDITY_BNB']} BNB)",callback_data='change_MINIMUM_LIQUIDITY_BNB')],[InlineKeyboardButton(f"🎯 هدف الربح 1 ({s['TAKE_PROFIT_THRESHOLD_1']}%)",callback_data='change_TAKE_PROFIT_THRESHOLD_1'),InlineKeyboardButton(f"📦 الكمية ({s['SELL_PERCENTAGE_1']}%)",callback_data='change_SELL_PERCENTAGE_1')],[InlineKeyboardButton(f"🎯 هدف الربح 2 ({s['TAKE_PROFIT_THRESHOLD_2']}%)",callback_data='change_TAKE_PROFIT_THRESHOLD_2'),InlineKeyboardButton(f"📦 الكمية ({s['SELL_PERCENTAGE_2']}%)",callback_data='change_SELL_PERCENTAGE_2')],[InlineKeyboardButton(f"🛑 وقف الخسارة ({s['STOP_LOSS_THRESHOLD']}%)",callback_data='change_STOP_LOSS_THRESHOLD')],[InlineKeyboardButton("🔙 العودة للقائمة الرئيسية",callback_data='main_menu')]]
    ...
    async def ask_for_new_value(self,update:Update,context:ContextTypes.DEFAULT_TYPE):
        ...
        prompts={"BUY_AMOUNT_BNB":"يرجى إرسال مبلغ الشراء الجديد بالـ BNB (مثال: 0.01):","GAS_PRICE_TIP_GWEI":"يرجى إرسال إكرامية الغاز الجديدة بالـ Gwei (مثال: 1):","SLIPPAGE_LIMIT":"يرجى إرسال نسبة الانزلاق السعري الجديدة (مثال: 49):","MINIMUM_LIQUIDITY_BNB":"يرجى إرسال الحد الأدنى للسيولة بالـ BNB (مثال: 5.0):","TAKE_PROFIT_THRESHOLD_1":"يرجى إرسال نسبة الهدف الأول للربح (مثال: 100):","SELL_PERCENTAGE_1":"يرجى إرسال نسبة البيع للهدف الأول (مثال: 50):","TAKE_PROFIT_THRESHOLD_2":"يرجى إرسال نسبة الهدف الثاني للربح (مثال: 300):","SELL_PERCENTAGE_2":"يرجى إرسال نسبة البيع للهدف الثاني (مثال: 100):","STOP_LOSS_THRESHOLD":"يرجى إرسال نسبة وقف الخسارة (مثال: -50):"}
...
class الراصد:
    ...
    async def check_connection_periodically(self):
        while True:
            ...
            try:
                block_number=await asyncio.wait_for(self.w3.eth.block_number,timeout=15.0)
...
class المدقق:
    ...
    async def محاكاة_عملية_بيع(self,token_address:str)->Tuple[bool,str]:
        ...
        try:
            ...
            await self.router_contract.functions.swapExactTokensForETHSupportingFeeOnTransferTokens(1,0,[checksum_token,Web3.to_checksum_address(WBNB_ADDRESS)],checksum_wallet,int(time.time())+120).call({'from':checksum_wallet})
...
class القناص:
    ...
    async def _approve_max(self,token_address:str):
        ...
        try:
            ...
            max_amount=2**256-1
            approve_tx=await token_contract.functions.approve(Web3.to_checksum_address(ROUTER_ADDRESS),max_amount).build_transaction({'from':self.account.address,'gasPrice':await self._get_dynamic_gas(),'gas':100000,'nonce':await self.nonce_manager.get_next()})
    ...
    async def تنفيذ_الشراء(self,token_address:str)->Dict[str,Any]:
        try:
            ...
            tx_params={'from':self.account.address,'value':bnb_amount_wei,'gas':self.bot_state.get('GAS_LIMIT',600000),'gasPrice':await self._get_dynamic_gas(),'nonce':await self.nonce_manager.get_next(),}
            tx=await self.router_contract.functions.swapExactETHForTokens(min_tokens,path,self.account.address,int(time.time())+120).build_transaction(tx_params)
...
class الحارس:
    ...
    async def _execute_sell(self,trade:Dict,amount_to_sell_wei:int,manual=False)->bool:
        ...
        try:
            ...
            tx_params={'from':self.account.address,'gas':self.bot_state.get('GAS_LIMIT',600000),'gasPrice':await self._get_dynamic_gas(),'nonce':await self.nonce_manager.get_next()}
            swap_tx=await self.router_contract.functions.swapExactTokensForETHSupportingFeeOnTransferTokens(amount_to_sell_wei,0,path,self.account.address,int(time.time())+300).build_transaction(tx_params)
...
async def main():
    ...
    try:
        ...
        block_number = await asyncio.wait_for(w3.eth.block_number, timeout=15)
        logging.info(f"✅✅✅ تم الاتصال بالشبكة بنجاح! رقم البلوك الحالي: {block_number} ✅✅✅")
    except asyncio.TimeoutError:
        logging.critical("❌❌❌ فشل الاتصال (Timeout). تأكد من صحة الرابط وأن الخادم يسمح بالاتصال.")
        return
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        logging.critical(f"❌ خطأ فادح وغير متوقع في البرنامج الرئيسي:", exc_info=True)
